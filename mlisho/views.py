...
def status(request):
    ...
    pastoralist = Pastoralist.objects.filter(user=request.user)
    return render_to_response("temp/status.html", {"pastoralist": pastoralist}, context_instance=RequestContext(request))
...
@login_required
def pastoralist_portal(request):
    form = PastoralistForm
    if request.method == 'POST':
        form = PastoralistForm(request.POST, request.FILES)
        if form.is_valid():
        ...
        form = PastoralistForm()
    images=Pastoralist.objects.all()
