...
var layer_well = new L.LayerGroup();
var array_markers = new Array();

/* create custom marker which will represent well in layer 'layer_well' */
...
function AddPointsToLayer() {
    for (var i=0; i<array_markers.length; i++) {
        array_markers[i].addTo(layer_well);
    }
} 

/* Get all well from DB and add them to layer:_well */
$.ajax({
	url: '/map/get_well/',
	type: 'GET',
	success: function(response) {
        $.each(eval(response), function(key, val) {	  
			...
			marker.bindPopup("<br><strong>Form:</strong>"+ fields.well_form + "<br><strong>Type:</strong>" + fields.well_type + "<br><strong>Compliance:</strong>"
				+ fields.well_compliance);
        	marker.addTo(map);
...
var map = L.map('map', {
	...
	layers: [OpenStreetMap, layer_well]
});
	...
	$.ajax({
		...
		success: function(response) {
	        ...
	        $.each(eval(response), function(key, val) {	  
	        	...
	        	marker.addTo(map);
	        	marker.bindPopup("<strong>Form:</strong><br>" + fields.well_form + "<br><strong>Type:</strong>"
				+ fields.well_type + "<br><strong>Compliance:</strong>"+ fields.well_compliance);
	        	array_markers.push(marker);
	        });
