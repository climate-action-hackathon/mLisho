...
// control that shows state info on hover
        ...
        function pop_weightedarea(feature, layer) {
            layer.on({
                mouseout: function(e) {
                    layer.setStyle(doStyleweightedarea(feature));
            ...
            var popupContent = '<table><tr><th scope="row">ID</th><td>' + Autolinker.link(String(feature.properties['ID'])) + '</td></tr><tr><th scope="row">GRIDCODE</th><td>' + Autolinker.link(String(feature.properties['GRIDCODE'])) + '</td></tr></table>';
            layer.bindPopup(popupContent);
        }
       ...
       $.getJSON(dssurl, function (data) {
            caldera.addData(data).setStyle(doStyleweightedarea);
            caldera.eachLayer(function (layer) { 
            layer.on('click', function (e) {
                var popup = "<strong>" + "Id : " + e.target.feature.properties.id + "GRIDCODE : " + e.target.feature.properties.gridcode +  "<br>" +  "</strong>";
                layer.bindPopup(popup).openPopup(e.latlng);
                //map.fitBounds(e.target.getBounds());
            }); 
            //locations.bindLabel(feature.properties['location_b'], { 'noHide': true });
